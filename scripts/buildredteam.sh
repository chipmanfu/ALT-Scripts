...
csc2path="/root/cobaltstrike/Profiles"
...
{
  MenuBanner
  intname=`ip --brief address show | grep -Ev "10.10.*.*|127.0.0" | awk '{print $1}'`
...
DecoyMenu()
{
  MenuBanner
   ...
   echo -e "\t\t$ltblue The default is$yellow www.google.com$ltblue"
...
ExecAndValidate()
{
  ...
  case $answer in
    q|Q) echo -e "$default"; exit 0;;
    b|B) case $opt in
           1|3) csc2CollectionMenu;; #campaign=; CampaignMenu;;
 ...
 ## REDELK - Disabled until one gets stood up
 ...
 # rm -f /etc/cron.d/redelk 2>&1 >/dev/null
  ...
  if [[ $setips == 1 ]]; then 
    ...
    echo -e "$green Finished!"
    echo -e "\t$green  Set /etc/resolv.conf to $SSDNS"
    echo "nameserver $SSDNS" > /etc/resolv.conf
...
## REDELK - Disabled until one is stood up
...
#    echo -e "$green Finished!"
  ...
  if (( $opt != 3 || $opt != 6 )); then
    ...
    echo -e "\n\t$white Step 1.$ltblue ping 198.41.0.4"
    echo -e "\t$white Step 2.$ltblue Copy ssh key to 198.41.0.4 ->  ssh-copy-id 198.41.0.4"
    echo -e "\t\t$ltgray NOTE: root password:$yellow toor"
    ...
    echo -e "\t\t automatically on the 198.41.0.4 DNS server \n" 
  fi
  if (( $opt == 4 )); then
    echo -e "\n\t$white Place files you want to host in /var/www/html"
  fi
  echo -e "$default"
  # Refreshes Bash if hostname is changed
  if [[ $hostnamesel != "" ]]; then
    exec bash
  fi
...
#    echo -e "\n\taccess_log /var/log/nginx/access-redelk.log redelklog;" >> $nginxconf
 ...
 #       echo -e "\t\t\tproxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;" >> $nginxconf
        echo -e "\t\t}" >> $nginxconf
        echo -e "\t\tssl_certificate /etc/ssl/certs/ssl-server.crt;" >> $nginxconf
        echo -e "\t\tssl_certificate_key /etc/ssl/private/ssl-server.key;" >> $nginxconf
        echo -e "\t}" >> $nginxconf
      done<$IPfile
    fi 
    if [[ $http == 1 ]]; then
...
BuildIntConfig()
{
    ...
    while read y; do
      ...
      case $div in
        3) octmod=` expr $oct4 + $addval - 2`;
	     ...
	     #Modified to account for simspace and everything being a class C
	     gwyIP=$oct1.$oct2.$oct3.1 
        ...
        1) octmod=` expr $oct2 + $addval`;
	     ...
	     gwyIP=$oct1.$randoct2.$randoct3.1
        ...
        0) octmod=` expr $oct1 + $addval`;
           for x in `shuf -i 1-254 -n $numips`; do   
             ...
             randoct3=`shuf -i 0-255 -n 1`
	     gwyIP=$randoct1.$randoct2.$randoct3.1
  ...
  # next add config for SimSpace control network
  echo "" >> $tempintfile
  echo "auto eth1" >> $tempintfile
  echo "iface eth1 inet dhcp" >> $tempintfile
}
